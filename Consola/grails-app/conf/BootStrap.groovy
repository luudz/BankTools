...
class BootStrap {
    ...
    def v5=new Variable(alias:"Regla005", columna:"Columna5", descripcion:"Esta es la quinta variable", categoria:"5", enabled:true).save(failonError:true);*/ 	
    	...
    	if(!Variable.count()){
    		...
    		cv1=new CatalogoVariables(nombre:"Regla001", descripcion:"Esta es la regla Regla001",enabled:true, variable:v1).save(failonError:true);
    		cv2=new CatalogoVariables(nombre:"Regla002", descripcion:"Esta es la regla Regla002",enabled:false, variable:v2).save(failonError:true);
   		 	cv3=new CatalogoVariables(nombre:"Regla003", descripcion:"Esta es la regla Regla003",enabled:true, variable:v3).save(failonError:true);
    		cv4=new CatalogoVariables(nombre:"Regla004", descripcion:"Esta es la regla Regla004",enabled:false, variable:v4).save(failonError:true);
    		cv5=new CatalogoVariables(nombre:"Regla005", descripcion:"Esta es la regla Regla005",enabled:true, variable:v5).save(failonError:true);
